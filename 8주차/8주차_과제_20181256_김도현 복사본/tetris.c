...
long int memorysize=0; //공간효율성을 확인하기 위한 글로벌 변수
int Play_Recommend=0; //play를 하면 recommend 실행, Recommend 하면 modified_recommend실행
...
int main(){
    ...
    modiroot=(ModiRecNode *)malloc(sizeof(ModiRecNode)); //새로 만든 modirecnode. 이걸 이용하여 modified_recommend의 효율성을 매우 높인다.
    modiroot->lv=0;
    modiroot->score=0;
...
void InitTetris(){
	int i,j;
    memorysize=0; //공간효율성 확인하기 위한 변수 초기화
	for(j=0;j<HEIGHT;j++)
		for(i=0;i<WIDTH;i++)
			field[j][i]=0;
...
//nextBlock[2]=rand()%7; //과제에서는 미리나오는 블록이 2개 이기 때문에 nextBlock을 2까지로 올린다. BLOCK_NUM 3으로 수정.
	...
	gameOver=0;
	timed_out=0;
    if(Play_Recommend==0) //만약 play가 선택되면 기존의 recommend가 실행되고 만약 recommend가 선택되면 modified_recommend 가 작동하도록 만들었다.
    {treeroot->f=field;
        recommend(treeroot);
    }
    else{
        modiroot->f=field;
        modified_recommend(modiroot);
    }
...
void play(){
    ...
    Play_Recommend=0; //Play or Recommend flag(0이 play를 의미)
	InitTetris();
	do{
		if(timed_out==0){
			alarm(1);
...
int recommend(RecNode *root){
    ...
    memorysize+=sizeof(*root); //생성되는 node들의 memory size를 누적 계산한다
    
	// user code
    int k=0; //child[k]를 탐색할 때 사용하는 index 변수 k 이다.
...
int modified_recommend(ModiRecNode *root)
{
    int recommendScore=0;// 미리 보이는 블럭의 추천 배치까지 고려했을 때 얻을 수 있는 최대 점수
    memorysize+=sizeof(*root); //생성되는 node들의 memory size를 누적 계산한다
    ...
    int rec_x; //현재 추천 x 좌표를 의미한다
    int rec_r; //현재 추천 r rotation 을 의미한다.
    int R; //블록당 실제 Rotation 개수
    ModiRecNode **child=root->c; //root->c 를 RecNode **child 로 선언했다.
        child[0]=(ModiRecNode *)malloc(sizeof(ModiRecNode));
        child[0]->f=(char (*)[WIDTH])malloc(sizeof(char)*WIDTH*HEIGHT);
    
    
     if(nextBlock[root->lv]==1) //Block 모양이 1 이면 rotation 4번
        R=4;
    else if(nextBlock[root->lv]==2)//Block 모양이 2 이면 rotation 4번
       R=4;
    else if(nextBlock[root->lv]==3)//Block 모양이 3 이면 rotation 4번
       R=4;
    else if(nextBlock[root->lv]==4)//Block 모양이 4 이면 rotation 1번
        R=1;
    else R=2; //나머지라면 rotation 2번
    
        
    
    
    for(rec_r=0;rec_r<R;rec_r++) //블록에 따른 R을 돌면서 먼저 for문을 두어 0부터 순차적으로 accumulatedScore를 계산해본다
    {
        for(rec_x=-1;rec_x<WIDTH;rec_x++) //다음은 rec_x 즉 x좌표를 가장 왼쪽부터 오른쪽까지 순차적으로 이동시키면서 accumulatedScore를 계산해본다
        {
            int accumulatedScore=0; //accumulatedScore 초기화
            ...
            while(1){ //계속해서 break 나오기 전까지  반복한다
               ...
               child[0]->lv=root->lv+1; //child의 leveㅣ을 root level 보다 하나 증가시킨다
        
            
            for( int i =0; i<HEIGHT;i++) //2중 for문을 이용하여 child Node의 field를 root의 field로 채운다.
                for(int j=0;j<WIDTH;j++)
                    child[0]->f[i][j]=root->f[i][j];
           
            accumulatedScore+=(AddBlockToField(child[0]->f,nextBlock[root->lv],rec_r,rec_y,rec_x)+DeleteLine(child[0]->f));
           //accumulatedScore는 안쪽 for문이 실행될때마다 초기화 된다. 그래서 여기에 rec_r,rec_y,rec_x 즉 해당 2중 for문으로 부터 구할 수 있는 rotation, y,x 좌표를 통해 score 점수를 계산해 저장한다
            
            
            
            //if(accumulatedScore<20) continue; //pruning 과정으로 만약 accumulatedScore가 20을 넘지 못한다면 그 node는 최대 점수가 안될 가능성이 매우 높기 때문에 고려하지 않는다. 그래서 다음 for문으로 바로 넘어간다. 시간적 효율성을 추구한 것이다.
        
            
            
            if(child[0]->lv<VISIBLE_BLOCKS) //만약 child 노드의 level 이 VISIBILE_BLOCKS 즉 최대 level 보다 작다면 재귀함수 recommend(child[k])를 넣어 child[k]의 또 child 노드들의 recommend 함수를 통해 계산된 점수를 누적해서 accumulatedScore에 저장한다.
                accumulatedScore+=modified_recommend(child[0]);
      
            accumulatedScore+=rec_y; //y 값의 가충치를 더해주어 최대한 y값이 큰 곳으로 다음 블록이 쌓여야 확률상 좋기 때문에 recommendX, recommendY, recommendR 이 바뀔 가능성이 높게 이 조건을 추가하였다.
            
            child[0]->score=accumulatedScore;//accumulatedScore를 child[0]의 score에 저장한다
        ...
        free(child[0]->f); //먼저 child[0]안에 있는 f 부터 메모리 free 시키고
        free(child[0]); //child[0] 를 free 시킨다
...
void recommendedPlay(){
    ...
    double dur;
    int command;
    clear();//시작 시 한번 지우고 시작
    act.sa_handler = AutoBlockDown; //play함수에서 BlockDown 함수를 플레이어 개입 없이 자동으로 일정 시간에 반복적으로 떨어질 수 있는 AutoBlockDown 함수를 새로 만들어 넣어주었다.
    sigaction(SIGALRM,&act,&oact);
    
    start=time(NULL);
    Play_Recommend=1; //recommendPlay 를 한다는 flag
    InitTetris(); //테트리스 초기화 함수
    do{
        if(timed_out==0){
            alarm(1);
            timed_out=1;
        ...
        command = wgetch(stdscr);
        switch(command){ //키보드 자판에서 q나 Q 두개만 받을 수 있도록 하였다
        case 'q':
        case 'Q':
            command = QUIT;
            break;
        ...
        if(ProcessCommand(command)==QUIT){//종료를 의미한다
            alarm(0);
            DrawBox(HEIGHT/2-1,WIDTH/2-5,1,10);
            move(HEIGHT/2,WIDTH/2-4);
            printw("Good-bye!!");
    ...
    stop=time(NULL);
    dur=(double)difftime(stop,start);
    alarm(0);
    getch();
    DrawBox(HEIGHT/2-1,WIDTH/2-5,1,10);
    move(HEIGHT/2,WIDTH/2-4);
...
void AutoBlockDown(int sig){
    // user code
    modiroot->lv=0;// treeroot 초기화
    modiroot->score=0; // treeroot 초기화
    if(CheckToMove(field, nextBlock[0],blockRotate,blockY+1,blockX))//checktomove를 통해 nextBlock[0] 블록을 밑으로 내릴 수 있는지 확인
    {
    blockY=blockY+1;// blockY 값을 하나 증가시킴
    }
if(blockY==-1) gameOver=1; //blockY==-1 이면 더이상 떨어질 곳이 없어 게임 종료
    ...
    modiroot->f=field; //treeroot 의 f 를 현재 field로 넣기
    modified_recommend(modiroot); //modified_recommend 함수 실행
...
DrawField(); //추가된 Field 다시 그리기
    DrawBlock(blockY,blockX,nextBlock[0], blockRotate, ' ');// 처음 떨어질때 어떤 블록인지 알 수있게 그려준다
    DrawBlock(recommendY, recommendX, nextBlock[0], recommendR,'R'); //추천 기능 블록을 r로 그려준다
    modiroot->lv=0;// treeroot 초기화
    modiroot->score=0; // treeroot 초기화
timed_out=0; //시간 다시 0
}
